...
void test_rlwe_bootstrap(){
  ...
  const uint64_t input_prec = 12,
                 input_base = 1<<input_prec,
                 message_scale = p_star/input_base;

  // generate input
  std::cout << "Generating Input...\n";
  IntPolynomial in_poly = polynomial_new_int_polynomial(N);
  ...
  for (size_t i = 0; i < N; i++) in_poly->coeffs[i] = (i%input_base)*message_scale + message_scale/2;
  RLWE in = rlwe_new_sample(in_poly, in_key);
  ...
  RLWE out = rlwe_alloc_sample(N, 1, h_ntt);
  MEASURE_TIME("", 1, "RLWE bootstrap", 
    rlwe_bootstrap_and_ks(out, in, tv, bks);
  )

  // decrypt
  std::cout << "Decrypting...\n";
  IntPolynomial m_rlwe = polynomial_new_int_polynomial(N);
  rlwe_phase(m_rlwe, out, in_key);
  // round
  for (size_t i = 0; i < N; i++) m_rlwe->coeffs[i] = ((m_rlwe->coeffs[i] + message_scale/2) / message_scale)%input_base;
  // print result
  print_array("Out:", m_rlwe->coeffs, N);
  // calculate error standard deviation
  double err = 0;
  for (size_t i = 0; i < N; i++){
    err += pow(mod_dist(m_rlwe->coeffs[i], i%input_base, input_base), 2);
  }
  printf("Error: 2^%f \n", log2(sqrt(err/N)));
  ...
  for (size_t i = 0; i < N; i++){
    out->b->coeffs[i] = intel::hexl::AddUIntMod(out->b->coeffs[i], message_scale/2, p_star);
  }

  // run rlwe bootstrap again
  std::cout << "Bootstrapping again...\n";
  RLWE out2 = rlwe_alloc_sample(N, 1, h_ntt);
  MEASURE_TIME("", 1, "RLWE bootstrap", 
    rlwe_bootstrap_and_ks(out2, out, tv, bks);
  )

  // decrypt
  std::cout << "Decrypting...\n";
  rlwe_phase(m_rlwe, out2, in_key);
  // round message
  for (size_t i = 0; i < N; i++) m_rlwe->coeffs[i] = ((m_rlwe->coeffs[i] + message_scale/2) / message_scale)%input_base;
  // print result
  print_array("Out:", m_rlwe->coeffs, N);
